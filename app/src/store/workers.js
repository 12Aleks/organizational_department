...
export default {
    actions: {
        async workerPhoto({commit, dispatch}, { photo, abbreviatedData }){
            try{
                const storageData =  await firebase.storage().ref(`/workers/${abbreviatedData}.jpg`)
        ...
        async receiveWorkerData({commit}, {workerName, departmentName, selectProcess}){
            const worker = workerName.slice(0,4).toLowerCase() + '_' + departmentName.slice(0,3).toLowerCase() + '_' + selectProcess.slice(0,3).toLowerCase();
            try{
